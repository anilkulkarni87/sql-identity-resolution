...
SELECT
    ...
    CASE WHEN c.cluster_size = 1 THEN 1.0
         ELSE ROUND(0.50 * (COALESCE(es.edge_diversity, 0) / GREATEST(1, (SELECT MAX(edge_diversity) FROM idr_work.cluster_edge_stats))) +
    ...
    CASE WHEN c.cluster_size = 1 THEN 'SINGLETON_NO_MATCH_REQUIRED'
         WHEN COALESCE(es.edge_diversity, 0) >= 2 THEN CONCAT(CAST(COALESCE(es.edge_diversity, 0) AS STRING), ' identifier types')
...
USING idr_work.cluster_confidence src
